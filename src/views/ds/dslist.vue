  ...
  <div>
    ...
    >
      ...
      <el-table-column label="操作" width="100" align="center">
        <template slot-scope="scope">
          <el-link  @click="()=>deleteDs(scope.row.metadata.namespace,scope.row.metadata.name )" >删除<i class="el-icon-delete el-icon--right"></i></el-link>
        </template>
      </el-table-column>
    </el-table>
  </div>
...
<script>
  import { getDsList,rmDs  } from '@/api/ds'
  ...
  export default {
    ...
    methods: {
      ...
      fetchData()
      {
        ...
        // 通过rest api 获取
        getDsList(this.namespace).then(response => {
        ...
        this.wsClient.onmessage = (e) => {
          if (e.data !== 'ping') {
            ...
            if (object.type === 'ds' && object.result.ns===this.namespace) {
